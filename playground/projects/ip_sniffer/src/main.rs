...
// This block can be used for implementing methods for struct and enum definitions, for trait implementations, or for providing associated functions that don’t take self as a parameter.
impl Arguments {
            ...
            Err(_) => {
                if flag.contains("-h") || flag.contains("-help") {
                    if args.len() == 2 {
                        ...
                        // Macros allow for code generation or metaprogramming at compile time, offering more flexibility than functions, such as accepting a variable number of arguments.
                        println!(
                            "Usage:
...
fn main() {
    ...
    let arguments = Arguments::new(&args).unwrap_or_else(|error| match error {
